...
from economy_helpers import (
    ...
    load_team_shop, load_angler_shop,
    find_shop_item, normalize_item_name,
)
...
def _get_angler_shop_items() -> list:
    return load_angler_shop()
...
def _build_angler_items_embed(member: discord.Member) -> discord.Embed:
    ...
    lines = [
        ...
        for it in _get_angler_shop_items()
    ]
    desc = sep + "\n" + "\n".join(lines) + "\n" + sep
    emb  = discord.Embed(
...
class AnglershopItemSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        name = self.values[0]
        item = next((it for it in _get_angler_shop_items() if it["name"] == name), None)
        if not item:
