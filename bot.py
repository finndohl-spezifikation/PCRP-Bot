...
# damit z.B. "Handy" das Item "ðŸ“±| Handy" sicher findet.
...
# â”€â”€ /Ã¼bergeben â€” UI: Item-Auswahl + Mengen-Modal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

class UebergebenMengeModal(discord.ui.Modal, title="Menge eingeben"):
    menge = discord.ui.TextInput(
        label="Menge",
        placeholder="Wie viele mÃ¶chtest du Ã¼bergeben? (z.B. 1)",
        min_length=1,
        max_length=6,
    )

    def __init__(self, item_name: str, empfaenger: discord.Member, giver_id: int):
        super().__init__()
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            menge_val = int(self.menge.value.strip())
        except ValueError:
            await interaction.response.send_message(
                "âŒ Bitte eine gÃ¼ltige Zahl eingeben.", ephemeral=True
            )
            return

        if menge_val <= 0:
            await interaction.response.send_message(
                "âŒ Die Menge muss mindestens 1 sein.", ephemeral=True
            )
            return

        eco        = load_economy()
        giver_data = get_user(eco, self.giver_id)
        inv        = giver_data.get("inventory", [])

        vorhandene = inv.count(self.item_name)
        if vorhandene < menge_val:
            await interaction.response.send_message(
                f"âŒ Du hast nur **{vorhandene}Ã—** **{self.item_name}** im Inventar, "
                f"mÃ¶chtest aber **{menge_val}** Ã¼bergeben.",
                ephemeral=True
            )
            return

        for _ in range(menge_val):
            inv.remove(self.item_name)

        receiver_data = get_user(eco, self.empfaenger.id)
        receiver_data.setdefault("inventory", []).extend([self.item_name] * menge_val)
        save_economy(eco)
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception):
        await interaction.response.send_message(
            "âŒ Ein Fehler ist aufgetreten. Bitte versuche es erneut.", ephemeral=True
        )


class UebergebenItemSelect(discord.ui.Select):
    def __init__(self, inventory_items: list, empfaenger: discord.Member, giver_id: int):
        self.empfaenger = empfaenger
        self.giver_id   = giver_id

        from collections import Counter
        counts  = Counter(inventory_items)
        options = [
            discord.SelectOption(
                label=item[:100],
                description=f"Vorhanden: {count}Ã—",
                value=item[:100],
            )
        ...
        super().__init__(
            placeholder="ðŸ“¦ WÃ¤hle ein Item aus deinem Inventar...",
            options=options,
            min_values=1,
            max_values=1,
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        modal    = UebergebenMengeModal(selected, self.empfaenger, self.giver_id)
        await interaction.response.send_modal(modal)


class UebergebenView(discord.ui.View):
    def __init__(self, inventory_items: list, empfaenger: discord.Member, giver_id: int):
        super().__init__(timeout=60)
        self.add_item(UebergebenItemSelect(inventory_items, empfaenger, giver_id))


# /Ã¼bergeben  â€” nur im Kanal 1490882592445304972 (Rucksack-Kanal)
...
@app_commands.describe(nutzer="EmpfÃ¤nger")
async def uebergeben(interaction: discord.Interaction, nutzer: discord.Member):
    role_ids = [r.id for r in interaction.user.roles]
    is_adm   = ADMIN_ROLE_ID in role_ids
    ...
    if nutzer.id == interaction.user.id:
        await interaction.response.send_message(
            "âŒ Du kannst nicht an dich selbst Ã¼bergeben.", ephemeral=True
        )
        return
    ...
    if not inventory:
        await interaction.response.send_message(
            "âŒ Dein Inventar ist leer. Du hast keine Items zum Ãœbergeben.",
            ephemeral=True
        )
        return

    view = UebergebenView(inventory, nutzer, interaction.user.id)
    await interaction.response.send_message(
        f"ðŸŽ’ WÃ¤hle ein Item das du an {nutzer.mention} Ã¼bergeben mÃ¶chtest:",
        view=view,
        ephemeral=True
    )
...
# â”€â”€ Einreise Modal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class EinreiseModal(discord.ui.Modal, title="Ausweis erstellen"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            typ_label     = "ðŸ¤µ Legale Einreise" if self.einreise_typ == "legal" else "ðŸ¥· Illegale Einreise"
            ...
            try:
                await interaction.response.send_message(
                    "âŒ Beim Erstellen deines Ausweises ist ein Fehler aufgetreten. Bitte versuche es erneut.",
                    ephemeral=True
                )
            except Exception:
                pass
...
class AusweisCreateModal(discord.ui.Modal, title="Ausweis erstellen (Admin)"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            try:
                await interaction.response.send_message(
                    "âŒ Beim Erstellen des Ausweises ist ein Fehler aufgetreten. Bitte versuche es erneut.",
                    ephemeral=True
                )
            except Exception:
                pass
...
class AusweisCreateEinreiseSelect(discord.ui.Select):
    def __init__(self, target_id: int):
        self.target_id = target_id
        options = [
            discord.SelectOption(label="Legale Einreise",   emoji="ðŸ¤µ", value="legal"),
            discord.SelectOption(label="Illegale Einreise", emoji="ðŸ¥·", value="illegal"),
        ]
...
async def ausweis_create(interaction: discord.Interaction, nutzer: discord.Member):
    ...
    if str(nutzer.id) in ausweis_data:
        await interaction.response.send_message(
            f"âŒ {nutzer.mention} hat bereits einen Ausweis. Bitte zuerst mit /ausweis-remove loeschen.",
            ephemeral=True
        )
        return
